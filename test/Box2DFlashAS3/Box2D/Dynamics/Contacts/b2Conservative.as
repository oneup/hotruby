...
public class b2Conservative{
	
	// Temp vars
	static private var R1:b2Mat22 = new b2Mat22();
	static private var R2:b2Mat22 = new b2Mat22();
	static private var x1:b2Vec2 = new b2Vec2();
	static private var x2:b2Vec2 = new b2Vec2();
	
	static public function Conservative(shape1:b2Shape, shape2:b2Shape):Boolean{
		var body1:b2Body = shape1.GetBody();
		var body2:b2Body = shape2.GetBody();
		
		//b2Vec2 v1 = body1->m_position - body1->m_position0;
		var v1X:Number = body1.m_position.x - body1.m_position0.x;
		var v1Y:Number = body1.m_position.y - body1.m_position0.y;
		//float32 omega1 = body1->m_rotation - body1->m_rotation0;
		var omega1:Number = body1.m_rotation - body1.m_rotation0;
		//b2Vec2 v2 = body2->m_position - body2->m_position0;
		var v2X:Number = body2.m_position.x - body2.m_position0.x;
		var v2Y:Number = body2.m_position.y - body2.m_position0.y;
		//float32 omega2 = body2->m_rotation - body2->m_rotation0;
		var omega2:Number = body2.m_rotation - body2.m_rotation0;
		
		//float32 r1 = shape1->GetMaxRadius();
		var r1:Number = shape1.GetMaxRadius();
		//float32 r2 = shape2->GetMaxRadius();
		var r2:Number = shape2.GetMaxRadius();
		
		//b2Vec2 p1Start = body1->m_position0;
		var p1StartX:Number = body1.m_position0.x;
		var p1StartY:Number = body1.m_position0.y;
		//float32 a1Start = body1->m_rotation0;
		var a1Start:Number = body1.m_rotation0;
		
		//b2Vec2 p2Start = body2->m_position0;
		var p2StartX:Number = body2.m_position0.x;
		var p2StartY:Number = body2.m_position0.y;
		//float32 a2Start = body2->m_rotation0;
		var a2Start:Number = body2.m_rotation0;
		
		//b2Vec2 p1 = p1Start;
		var p1X:Number = p1StartX;
		var p1Y:Number = p1StartY;
		//float32 a1 = a1Start;
		var a1:Number = a1Start;
		//b2Vec2 p2 = p2Start;
		var p2X:Number = p2StartX;
		var p2Y:Number = p2StartY;
		//float32 a2 = a2Start;
		var a2:Number = a2Start;
		...
		var invRelativeVelocity:Number = 0.0;
		//bool hit = true;
		var hit:Boolean = true;
		//b2Vec2 x1, x2; moved to static var
		for (var iter:int = 0; iter < maxIterations; ++iter)
		{
			// Get the accurate distance between shapes.
			//float32 distance = b2Distance.Distance(&x1, &x2, shape1, shape2);
			var distance:Number = b2Distance.Distance(x1, x2, shape1, shape2);
			...
			if (iter == 0)
			{
				...
				var relativeVelocity:Number = (dX*(v1X-v2X) + dY*(v1Y - v2Y)) + Math.abs(omega1) * r1 + Math.abs(omega2) * r2;
			...
			var ds:Number = distance * invRelativeVelocity;
			//float32 s2 = s1 + ds;
			var s2:Number = s1 + ds;
